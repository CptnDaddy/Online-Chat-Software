        ...
        while ($row = $result->fetch_array(MYSQLI_BOTH)) {
			...
			if($stmt2->field_count > 0) {
				...
				while ($row2 = $result2->fetch_array(MYSQLI_BOTH)) {
		            echo '<li onclick="changeRoom(' . $row['id'] . ')" class="collection-item room"><span class="badge"><a class="modal-trigger" href="#modal1">' . $row2['currentusers'] . '/' . $row['maxusers'] . '</a></span>' . $row['name'] . '</li>';
				}
			} else {
